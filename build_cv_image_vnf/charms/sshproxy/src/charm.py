...
class SshproxyCharm(SSHProxyCharm):
    ...
    def on_start_action(self, event):
        ...
        if self.unit.is_leader():
            ...
            proxy.run("sudo echo \"deb [arch=amd64] https://osm-download.etsi.org/repository/osm/debian/ReleaseNINE stable IM devops osmclient\" | sudo tee -a /etc/apt/sources.list")
    ...
    def on_ip_static_vm_action(self, event):
        ...
        if self.unit.is_leader():
            ...
            proxy.run("python3 {}cv_app/ip_static_vm.py".format(self.github_dir))
